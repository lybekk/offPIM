  ...
  methods: {
    fillDataTable: async function () {
      ...
      try {
        ...
        ldb.push({ key: "Local DB Name", value: result.db_name });
        ldb.push({ key: "PouchDB DB Adapter", value: result.adapter });
        ldb.push({ key: "Auto Compaction", value: result.auto_compaction });
        ldb.push({ key: "Update sequence", value: result.update_seq });
        ldb.push({ key: "Local documents", value: result.doc_count });
      } catch (err) {
